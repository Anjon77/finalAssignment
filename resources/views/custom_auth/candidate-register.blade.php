<x-guest-layout>
    ...
    <h1 class="text-center"
        style="font-size: xx-large; 
            border-bottom: 1px solid black; margin-bottom: 20px; background-color: lightgray">
        <strong>Candidate
            Registration</strong>
    </h1>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        {{-- <div>
            ...
            <x-text-input id="role" class="block mt-1 w-full" type="text" name="role" :value="old('role')"
                required autofocus autocomplete="role" value="Role as a Candidate" hidden readonly />
            <x-input-error :messages="$errors->get('role')" class="mt-2" />
        </div> --}}


        <input type="hidden" name="role" value="candidate">
        {{-- Status hidden input field --}}
